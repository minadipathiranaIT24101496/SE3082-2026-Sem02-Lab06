/* Molecular Dynamics simulation of argon atoms in a box with periodic
 * boundary conditions (Lennard-Jones potential).  Structure follows the
 * classic EPCC MolDyn exercise.
 *
 * Serial  : gcc -O2 -fopenmp main.c md_util.c forces.c     -o md_serial
 * Parallel: gcc -O2 -fopenmp main.c md_util.c forces_omp.c -o md_omp
 */
#include <omp.h>
#include <stdio.h>
#include <math.h>
#include "moldyn.h"

double epot, vir, count;
double x[npart*3], vh[npart*3], f[npart*3];

int main(void) {
  int move;
  double ekin, vel, sc;
  double tstart, tstop, tcalc;

  /* Parameter definitions */
  double den    = 0.83134;
  double side   = pow((double)npart / den, 0.3333333);
  double tref   = 0.722;
  double rcoff  = (double)mm / 4.0;
  double h      = 0.064;
  int    irep   = 10;
  int    istop  = 20;
  int    iprint = 5;
  int    movemx = 20;

  double a      = side / (double)mm;
  double hsq    = h * h;
  double hsq2   = hsq * 0.5;
  double tscale = 16.0 / ((double)npart - 1.0);
  double vaver  = 1.13 * sqrt(tref / 24.0);

  /* Generate fcc lattice for atoms inside box */
  fcc(x, npart, mm, a);
  /* Initialise velocities and forces (which are zero in fcc positions) */
  mxwell(vh, 3*npart, h, tref);
  dfill(3*npart, 0.0, f, 1);

  printf("\n    Molecular Dynamics Simulation example program\n");
  printf("    ---------------------------------------------\n");
  printf("    number of particles is ............ %6d\n",    npart);
  printf("    side length of the box is ......... %13.6f\n", side);
  printf("    cut off is ........................ %13.6f\n", rcoff);
  printf("    reduced temperature is ............ %13.6f\n", tref);
  printf("    basic timestep is ................. %13.6f\n", h);
  printf("    temperature scale interval ........ %6d\n",    irep);
  printf("    stop scaling at move .............. %6d\n",    istop);
  printf("    print interval .................... %6d\n",    iprint);
  printf("    total no. of steps ................ %6d\n",    movemx);
  printf("    OpenMP threads .................... %6d\n\n",  omp_get_max_threads());
  printf("   move      k.e.       p.e.      tot.e.    temp.     pres.     vel.    rp\n");

  tstart = omp_get_wtime();

  for (move = 1; move <= movemx; move++) {
    /* 1. Move the particles and partially update velocities */
    domove(3*npart, x, vh, f, side);
    /* 2. Compute forces in the new positions, accumulate virial & p.e. */
    forces(npart, x, f, side, rcoff);
    /* 3. Scale forces, complete update of velocities and compute k.e. */
    ekin = mkekin(npart, f, vh, hsq2, hsq);
    /* 4. Average the velocity and temperature scale if desired */
    vel = velavg(npart, vh, vaver, h);
    if (move < istop && fmod(move, irep) == 0) {
      sc = sqrt(tref / (tscale * ekin));
      dscal(3*npart, sc, vh, 1);
      ekin = tref / tscale;
    }
    /* 5. Sum to get full potential energy and virial, print */
    if (fmod(move, iprint) == 0)
      prnout(move, ekin, epot, tscale, vir, vel, count, npart, den);
  }

  tstop = omp_get_wtime();
  tcalc = tstop - tstart;
  printf("\nTime = %f s\n", tcalc);
  return 0;
}
